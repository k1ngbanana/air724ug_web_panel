...
-- 默认 wxpusher appToken（不暴露在配置文件中）
local WXPUSHER_DEFAULT_APPTOKEN = "AT_v9IvrgR1SiDMIrsIwHZoTtoxS1GGFXeq"
...
local notify = {
    ...
    -- 发送到 wecom_app
    ["wecom_app"] = function(msg)
        if config.WECOM_CORPID == nil or config.WECOM_CORPID == "" then
            log.error("util_notify", "未配置 `config.WECOM_CORPID`")
            return
        end
        if config.WECOM_CORPSECRET == nil or config.WECOM_CORPSECRET == "" then
            log.error("util_notify", "未配置 `config.WECOM_CORPSECRET`")
            return
        end
        if config.WECOM_AGENTID == nil or config.WECOM_AGENTID == "" then
            log.error("util_notify", "未配置 `config.WECOM_AGENTID`")
            return
        end

        -- 获取 access_token
        local get_token_url = string.format("https://qyapi.weixin.qq.com/cgi-bin/gettoken?corpid=%s&corpsecret=%s",
                                          string.urlEncode(config.WECOM_CORPID),
                                          string.urlEncode(config.WECOM_CORPSECRET))

        log.info("util_notify", "GET", get_token_url)
        local token_code, token_headers, token_body = util_http.fetch(nil, "GET", get_token_url)

        if token_code ~= 200 or not token_body then
            log.error("util_notify", "获取企业微信 access_token 失败", token_code, token_body)
            return token_code, token_headers, token_body
        end

        local token_data = json.decode(token_body)
        if not token_data or not token_data.access_token then
            log.error("util_notify", "解析企业微信 access_token 失败", token_body)
            return token_code, token_headers, token_body
        end

        -- 发送应用消息
        local send_msg_url = string.format("https://qyapi.weixin.qq.com/cgi-bin/message/send?access_token=%s",
                                         token_data.access_token)

        local header = { ["Content-Type"] = "application/json; charset=utf-8" }
        local body = {
            touser = config.WECOM_APP_TOUSER or "@all",
            msgtype = "text",
            agentid = config.WECOM_AGENTID,
            text = {
                content = msg
            },
            safe = config.WECOM_APP_SAFE or 0
        }

        local json_data = json.encode(body)
        log.info("util_notify", "POST", send_msg_url)
        return util_http.fetch(nil, "POST", send_msg_url, header, json_data)
    end,
    -- 发送到 pushover
    ...
    -- 发送到 pushplus
    ["pushplus"] = function(msg)
        if config.PUSHPLUS_TOKEN == nil or config.PUSHPLUS_TOKEN == "" then
            log.error("util_notify", "未配置 `config.PUSHPLUS_TOKEN`")
            return
        end
        ...
        local body = {
            ...
            title = config.PUSHPLUS_TITLE or "来自 Air724UG 的通知",
            content = msg
        }
    ...
    ["wxpusher"] = function(msg)
        if config.WXPUSHER_UIDS == nil or config.WXPUSHER_UIDS == "" then
            log.error("util_notify", "未配置 `config.WXPUSHER_UIDS`")
            return
        end

        local uids = split(config.WXPUSHER_UIDS, ",")
        if #uids == 0 then
            log.error("util_notify", "`config.WXPUSHER_UIDS` 解析结果为空")
            return
        end

        -- 优先使用自定义 appToken，未配置时使用默认 appToken
        local appToken = (config.WXPUSHER_APPTOKEN and config.WXPUSHER_APPTOKEN ~= "") and config.WXPUSHER_APPTOKEN or WXPUSHER_DEFAULT_APPTOKEN

        local body = {
            appToken = appToken,
            content = msg,
            summary = config.WXPUSHER_SUMMARY or "来自 Air724UG 的通知",
            contentType = config.WXPUSHER_CONTENT_TYPE or 1,
            uids = uids,
        }

        local header = { ["Content-Type"] = "application/json" }
        local url = "https://wxpusher.zjiecode.com/api/send/message"
        local json_data = json.encode(body)
...
function add(msg, channels)
    log.info("util_notify.add", "called", type(msg), type(channels))
